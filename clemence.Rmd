...
```{r, message=FALSE, warning=FALSE}
tps <- 1:356
# Initialisation d'une matrice pour stocker les résultats pour chaque colonne
smoothed_data <- list()

for (i in 2:34) {
  y <- as.vector(t(unlist(data_long[, i]))) # Sélectionne chaque colonne individuellement
  ...
  # Stocke les résultats dans la matrice
  smoothed_data[[i-1]] <- smoothdata$fd
}

# Maintenant, smoothed_data contient les données lissées pour les 33 colonnes

```

```{r}
matplot(smoothed_data)
```
...
plotfit.fd(y,# valeur de consommation pour un pays
           tps, # les dates
           smoothdata, # nos données estimées
           # 1 à 356 
           pch=20,
           cex=0.5,
...
```{r}
fhatprim = eval.fd(tps,smoothed_data,Lfdobj=1) # dérivé première
# eval.fd : évalue sur les dates
fhatpprim = eval.fd(tps,smoothed_data,Lfdobj=2) # dérivé seconde
...
[Pour un Etat]{.underline}

Nous représentons les données brutes et les données lissées pour un Etat

```{r}
fhatsmooth = eval.fd(1:356,smoothdata$fd)
par(mfrow=c(1,2))
matplot(1:356,data_long[,3],type="l",lty=1,ylab="",main="donnees brutes")
matplot(1:356,fhatsmooth,type="l",lty=1,ylab="",main="donnees lissées")
```
...
# ggplot(data, aes(x = Date, y = Consommation, color = Etat)) +
#   geom_line() +
#   geom_line(aes(x = mean_smooth_data$fdnames$time, y = mean_smooth_data$params))+
#   labs(x = "Date", y = "Consommation d'énergie", title = "Consommation d'énergie par Etat") +
#   theme_minimal()

matplot(fhatsmooth,col="gray",type="l",xlab="jours",ylab="temperature")
lines(mean_smooth_data,lwd=2)
```
### Ecart-type fonctionnel

```{r}
sd_smooth_data= sd.fd(smoothdata$fd) #ecart-type (objet fonctionnel fd)
```

```{r}
matplot(fhatsmooth,col="gray",type="l",xlab="jours",ylab="temperature")
fnmoy = eval.fd(1:365,mean_smooth_data)
fnsd  = eval.fd(1:365,sd_smooth_data)
lines(fnmoy,lwd=2)
