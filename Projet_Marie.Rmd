...
Nous allons à présent travailler sur le signal symétrisé. \

Pour pouvoir procéder à une **décomposition dans une base d'ondelettes avec x moments nuls**, nous allons tester plusieurs valeurs pour pouvoir s'approcher du nombre optimal de moments nuls. 


```{r}
# Décomposition en ondelettes avec 8 moments nuls (ajustez le nombre si nécessaire)
coefs <- wd(ysym, filter.number = 8, family = "DaubLeAsymm")

# Tracer les coefficients d'ondelettes
plot(coefs, scaling = "by.level")
```
# A FAIRE 
- ne pas normaliser les données
- réduire les données (soustraire la moyenne)
- choisir bases de fourier avec bcp de noeuds (attention à ne pas faire de sur-apprentissage)
- choisir bases de splines avec régression pénalisée pour pouvoir faire l'ACP
