...
Le problème ici est que la longueur des courbes de consommation n'est pas de la forme $2^J$. Pour la courbe de température de longueur 365, on symétrise le signal à la fin pour atteindre une longueur de 512. 
On se ramènera à la fin au signal de départ de taille 365.
...
plot(1:361,y,type="l",main="Signal de départ", ylab = "Consommation", xlab="Jour")
...
```{r}
splbasis = create.bspline.basis(rangeval = c(1,361),norder=4,breaks= seq(1,15,by=1))
# définition une base de B-spline entre 1 et 18 (âge min et max) 
# ordre 4 (classique) 
# breaks : on prend les âges de mesure comme point de rupture
...
gcv = 1:50 # grille de lambda

for (i in 1:50){
  
  lambda = exp(i-10) 
  # on teste exp(1-10), exp(1-9),... => permet d'avoir des petites / grandes valeurs
  
  fdpar = fdPar(splbasis,Lfdobj = 2,lambda=lambda) # calcul de la valeur de pénalité
  # splbasis : base utilisée
...
base de fourier avec bcp de noeuds

vérifier pourquoi c'est pas 356
